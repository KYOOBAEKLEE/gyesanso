...
1. 연차수당 계산기 (annual-leave.html) — 연차 개수 계산(입사일 기준) + 미사용 연차수당. hourly/severance와 연계
2. 부동산 중개수수료 계산기 (fee.html) — 매매/전세/월세 상한요율표 기반
3. 사용자가 도메인/애드센스 코드 주면: canonical, 애드센스 태그, sitemap/robots 실주소 반영 (REPLACE-DOMAIN 치환)
4. 백로그 소진 시: 각 페이지 FAQ 확장, 새 계산기(실업급여, 국민연금 예상수령액 등) 검색량 순 추가
