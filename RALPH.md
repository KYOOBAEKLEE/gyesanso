...
1. 실업급여 계산기 (unemployment.html) — 이직 전 평균임금 60%, 상한 66,000원/일·하한, 소정급여일수(연령/가입기간)표
2. 사용자가 도메인/애드센스 코드 주면: canonical, 애드센스 태그, sitemap/robots 실주소 반영 (REPLACE-DOMAIN 치환)
3. 백로그 소진 시: 각 페이지 FAQ 확장, 새 계산기(국민연금 예상수령액, 증여세 등) 검색량 순 추가

## 주의 (다음 반복의 나에게)
- 실업급여 상하한액은 매년 바뀜 — 구현 시 최신 고시 확인 필요, 모르면 RATES처럼 상수로 빼고 주석
- 계산기 10종이면 충분히 두터움. 이후로는 신규 페이지보다 기존 페이지 콘텐츠 보강(검색 의도별 FAQ)이 수익에 더 효과적
